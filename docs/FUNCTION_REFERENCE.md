...
Converts a label into a CSS-friendly slug.
...
- Used for skin-type classes such as `skin-oily`.
...
### `selectedConcernsForMap()`

Combines selected concerns and real special conditions for the face visual.

What it does:

- Includes face/body concerns.
- Includes lips/eyes concerns.
- Includes special conditions except `None`.

Why it exists:

- The live face preview should reflect all visible selected profile traits.

### `markerHtml(marker)`

Builds one HTML marker for the face scan overlay.

What it does:

- Uses marker type, label, and percentage position.
- Escapes the label.

Why it exists:

- Converts concern visual configuration into DOM elements.

### `completionState()`

Calculates how complete the testing profile is.

What it checks:

- Skin type selected.
- At least one concern selected.
- Special condition selected.
- Age selected.
- Gender selected.

Why it exists:

- Drives the completion bar and disables product refresh until a concern exists.

### `updateSkinMap()`

Updates the face scan preview.

What it does:

- Chooses male/female portrait.
- Adds visual state classes for dryness, dullness, and sensitivity.
- Renders concern markers.
- Updates narrative text and legend pills.
- Updates completion progress.
- Enables/disables the product refresh button.

Why it exists:

- Makes profile testing feel closer to the Roopsee experience instead of a plain form.
